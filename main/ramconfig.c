...
// Set by applyConfig() so the next fixRetailRAMConfig() call is skipped.
static bool suppressRetailFix = false;

void suppressRetailRAMFixOnce(void) {
	suppressRetailFix = true;
}

void fixRetailRAMConfig(void) {
	// Honour an explicit user-applied configuration exactly once.
	if (suppressRetailFix) {
		suppressRetailFix = false;
		return;
	}
...
static void applyConfig(
	...
	// The user explicitly chose this configuration, so don't let the RAM
	// tester menu's "accidental 8MB retail BIOS" correction immediately
	// undo it. Without this, selecting 8MB and applying it silently
	// snapped straight back to 2MB: applyConfig() writes DRAM_CTRL and
	// then enters the tester menu, whose fixRetailRAMConfig() call sees
	// "8MB + 1 bank" and rewrites it to 2MB.
	suppressRetailRAMFixOnce();
...
void enterRAMConfigMenu(
	...
	state->menuCursor  = findFirstSelectableItem(ramConfigMenu);
}
