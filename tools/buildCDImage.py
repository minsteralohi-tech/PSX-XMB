...
JSON file, allowing for customization of the disc's layout. Additional options
to inject "license" data required by certain PS1 BIOS versions into the first 16
sectors of the image are also provided. Requires pycdlib to be installed.
"""
...
def restoreISOFileOrder(
	iso:         PyCdlib,
	isoEntries:  Iterable[DirectoryRecord | Inode],
...
## PS1 system area license injection

REGION_OFFSET: int = 2048 * 4
REGION_LENGTH: int = 72
LOGO_OFFSET:   int = 2048 * 5
LOGO_LENGTH:   int = 2048 * 7

def injectSystemAreaData(file: BinaryIO, region: str = "", logo: bytes = b""):
	if region:
		regionData: bytes = region.encode("shift-jis")

		if len(regionData) > REGION_LENGTH:
			raise ValueError("region string is too long for system area")

		file.seek(REGION_OFFSET)
		file.write(regionData)

	if logo:
		if len(logo) > LOGO_LENGTH:
			raise ValueError("logo file is too large for system area")

		file.seek(LOGO_OFFSET)
		file.write(logo)
...
def main():
	...
	iso.new(
		interchange_level  = int(configFile.get("isoLevel", 1)),
		...
		joliet             = None,
		rock_ridge         = None,
		xa                 = bool(configFile.get("xa", False)),
		udf                = None
	)
	...
	systemArea: dict[str, str] = configFile.get("systemArea", {})
	logo:       bytes          = b""

	if "logo" in systemArea:
		with open(sourceDir / systemArea["logo"], "rb") as file:
			logo = file.read()
	...
	with args.output as file:
		...
		injectSystemAreaData(file, systemArea.get("region", ""), logo)
